...
function main(rc: replicadLib, par: parType) {
  ...
  let boxPeriBds = boxPeri.boundingBox.bounds;
  ...
  // let scMrg = boxTh + insert.d / 2;
  // let boxScrews = [
  //   V(boxPeriBds[0]).add(V([1, 1]).multiply(scMrg)),
  //   V([boxPeriBds[1][0], boxPeriBds[0][1]])
  //     .add(V([-1, 1]).multiply(scMrg)),
  //   V(boxPeriBds[1]).add(V([-1, -1]).multiply(scMrg))
  //     .sub(V([0, picodr.boundingBox.height + innRnd])),
  //   V([boxPeriBds[0][0], boxPeriBds[1][1]])
  //     .add(V([1, -1]).multiply(scMrg))
  // ];
  ...
  // let keymPlate = keym.plateModule();
  // let keymBoard = keym.boardModule();
  //
  // return [
  //   topShell
  // ].concat(
  //   [picoModPlaced.plateModule()].concat(
  //     keymOffs.map(v => keymPlate.clone().translate(v))
  //   ).map(s => s.translateZ(-boxTh - overPlate)),
  //   [picoModPlaced.boardModule()].concat(
  //     keymOffs.map(v => keymBoard.clone().translate(v))
  //   ).map(s => s.translateZ(-boxTh - overPlate - belowPlate)),
  // )
